...
| `mtime` | int | 通用修改时间戳（毫秒） | Dingtalk/LocalDrive Source | HindsightDestination |
...
| `dingtalk_extension` | str | 钉钉原始扩展名（内容转换前） | DingtalkSource |
| `dingtalk_update_time` | int | 钉钉文档更新时间戳（毫秒，已弃用，改用 mtime） | DingtalkSource |
