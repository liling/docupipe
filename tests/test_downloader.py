...
def test_download_single_doc(mock_client, output_dir, display):
    ...
    mock_client.list_nodes.return_value = [
        {"nodeId": "n1", "name": "文档1", "nodeType": "doc", "contentType": "ALIDOC", "extension": "adoc"},
    ]
...
def test_download_nested_folders(mock_client, output_dir, display):
    ...
    mock_client.list_nodes.side_effect = [
        [{"nodeId": "f1", "name": "子文件夹", "nodeType": "folder", "hasChildren": True}],
        [{"nodeId": "n1", "name": "嵌套文档", "nodeType": "doc", "contentType": "ALIDOC", "extension": "adoc"}],
    ]
...
def test_download_file_type(mock_client, output_dir, display):
    mock_client.get_space_info.return_value = {"id": "sp1", "name": "测试知识库"}
    mock_client.list_nodes.return_value = [
        {"nodeId": "n2", "name": "报告", "nodeType": "file", "contentType": "FILE", "extension": "pdf"},
    ]
...
def test_download_resume_skips_existing(mock_client, output_dir, display):
    mock_client.get_space_info.return_value = {"id": "sp1", "name": "测试知识库"}
    mock_client.list_nodes.return_value = [
        {"nodeId": "n1", "name": "已有文档", "nodeType": "doc"},
    ]
    ...
    state_dir = space_dir / ".state"
    state_dir.mkdir(parents=True, exist_ok=True)
    import hashlib
    content = "old content".encode()
    file_hash = hashlib.sha256(content).hexdigest()
    state_file = state_dir / "download_state.json"
    state_file.write_text(json.dumps({"n1": file_hash}), encoding="utf-8")
